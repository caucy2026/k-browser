...
- 首版按“语义块与视口相交”选择起点，长段落段首滚出屏幕后仍可能回读隐藏文字；`320fc38` 改为
  DOM Range 逐字符行框定位，从 D2 工具栏下方实际显示的第一行、第一字起读。
- 跟随逻辑保存首行锚点、当前块和块内字符游标，按当前朗读行自动平滑滚动；重复短语不再跳回段首，
  暂停后重建当前段仍保持原位置。
- 改动整理为 `0031-add-webpage-read-aloud.patch` 与
  `0032-start-read-aloud-at-first-visible-line.patch`，补丁入口更新为 0001–0020 + 0025–0032。
- 本地 `1.2.2-rc2` 完整 Release、R8、资源优化及 APK v2/v3 验签通过。源码脚本和 APK 内脚本均通过
  “隐藏段首 / D2 首行 / 屏底下一行”回归，正式 APK SHA-256 为
  `af6e9ecfd0429b55d6fc73158423681dd8cde340e72bf8a56f180838a6107f5d`。
- 正式版本已覆盖安装到 `192.168.3.63:5555` 并确认 `versionName=1.2.2-rc2`；安装未抢占当时两屏
  其他测试应用，未执行的真机声音对照保持为待复测，不记为通过。
