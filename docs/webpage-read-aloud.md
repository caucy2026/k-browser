...
- 单屏模式复用同一入口、主页和朗读状态机。
- 点击“朗读”后从当前可见正文开始，不从已经滚过的页面顶部重读。
...
脚本过滤隐藏元素、脚本、样式和模板，合并空白并去重，从第一个与当前视口相交的块开始返回，最多
500,000 字符。
...
2. 在公开长页面滚到中部后点“朗读”，确认不从页首重读。
3. 连续执行朗读、暂停、继续、停止，检查状态和日志一致。
4. 朗读中点击链接、主页、返回和退出，确认立即停声且双屏生命周期仍联动。
5. 断网或清除测试机系统语音参数时，只验证明确降级/报错；禁止在代码中补写任何凭据。
